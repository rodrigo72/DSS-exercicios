...
public class TextUI {
    ...
    private void menuPrincipal() {
        Menu menu = new Menu(new String[]{
                ...
                "Operações sobre Salas"
        });

        // Registar pré-condições das transições
        menu.setPreCondition(3, ()->this.model.haAlunos() && this.model.haTurmas() && this.model.haSalas());
        ...
        menu.setHandler(6, ()->gestaoDeSalas());

        menu.run();
    }

    private void gestaoDeSalas() {
        Menu menu = new Menu("Gestão de Salas", new String[]{
           "Adicionar Sala",
           "Consultar Sala",
           "Listar Salas"
        });

        menu.setHandler(1, ()->adicionarSala());
        menu.setHandler(2, () ->consultarSala());
        menu.setHandler(3, ()->listarSalas());
    ...
    private void mudarSalaDeTurma() {
        try {
            ...
            if (this.model.existeTurma(tid)) {
                ...
                int cap = scin.nextInt();
                scin.nextLine();    // Limpar o buffer depois de ler o inteiro
                if (!this.model.alteraSalaDeTurma(tid, new Sala(sala, edif, cap)))
    ...
    private void adicionarSala() {
        try {
            ...
            if (!this.model.existeSala(num)) {
                System.out.println("Edificio da nova sala: ");
                String ed = scin.nextLine();
                System.out.println("Capacidade da novo sala: ");
                String cap = scin.nextLine();
                this.model.adicionaAluno(new Aluno(num, ed, cap));
                System.out.println("Sala adicionado");
            } else {
